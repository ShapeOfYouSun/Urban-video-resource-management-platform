...
// 高级检索modal selector
    let modalName = window.$('#file-name input'),
        ...
        modalFooterCancel = window.$('#batch-import--tool .modal-footer button.cancel'),
        modalFooterConfirm = window.$('#batch-import--tool .modal-footer button.confirm'),
...
// 编辑/详情 modal selector
    let infoModalContent = '#file-info .modal-content',
        ...
        infoModalUsername = '#file-info-username',
        infoModalAccountCode = '#file-info-accountCode',
        infoModalUserRealname = '#file-info-names',
        infoModalSex = '#file-info-sex',
    ...
    // 编辑单个文件的信息/查看单个文件的详情
    function operateSingleFile(type, accountId) {
        ...
        $http.handle('userManage_userInfo', {accountId: accountId}, undefined, undefined, function (resp) {
            ...
            // 文件履历信息
            $http.handle('fileManage_fileRecord', {dataId: accountId}, undefined, undefined, function (recordResp) {
            ...
            // 账户ID
            window.$(infoModalName).replaceWith(window.$('<span></span>').attr('id', 'file-info-name').text(resp['accountId'] || ''));

            // 用户名
            window.$(infoModalUsername).replaceWith(window.$('<span></span>').attr('id', 'file-info-username').text(resp['userName'] || ''));

            // 用户编码
            window.$(infoModalAccountCode).replaceWith(window.$('<span></span>').attr('id', 'file-info-accountCode').text(resp['accountCode'] || ''));

            // 真实姓名
            window.$(infoModalUserRealname).replaceWith(window.$('<span></span>').attr('id', 'file-info-names').text(resp['name'] || ''));

            // 性别
            window.$(infoModalSex).replaceWith(window.$('<span></span>').attr('id', 'file-info-sex').text(resp['sex'] || ''));
    ...
    Object.defineProperties(infoModal, {
        accountId: {
            ...
            get: function () {
                return window.$(infoModalUsername).text();
            }, set: function (newValue) {
                window.$(infoModalUsername).text(newValue);
            }
        },industry: {
            get: function () {
            ...
            get: function () {
                ...
                window.$(infoModalSuffix).text(newValue);
            }
        }, origin: {
            get: function () {
    ...
    $http.handle('fileManage_departmentByIndustry', {}, undefined, undefined, function (res) {
        ...
        //     });
        // });
        let roleName = [{"text":"运营人员","id":'6014204003'},{"text":"机构管理员","id":'6014204006'},{"text":"平台管理员","id":'6014204004'}];
        roleName.forEach(function (currentValue)  {
            _departmentByIndustry[currentValue['id']] = getNodeAllChildren(currentValue); // 初始化industry - department信息
            // modalIndustry.append(window.$('<option></option>')).attr('value', currentValue['id']).text(currentValue['text']); // 初始化modal层行业下拉框信息
            modalIndustry.append(window.$('<option>'+currentValue['text']+'</option>'))
            layui.use('form', function () {
                let form = layui.form;
                form.render();
            });
    ...
    $table = new Table(window.$('#file-table'), {
        ...
        queryParams: function (params) {
            return {
                ...
                role: params.role,
                regDate: params.data,   //注册日期
                orgId: page.orgId, //机构名称
                index:params.index,
                // videoName: page.name,
        ...
        columns: [{
            ...
            formatter: function (value, row, index, field) {
                let result = window.$('<div></div>').addClass('operation');
                result.append('<div class="play" title="密码重置"><i class="fas fa-eye"></i></div>');
                result.append(window.$('<div class="edit" title="编辑"><i class="far fa-edit"></i></div>'));
                return result[0].outerHTML;
            },
            events: {
                'click .play': function (event, value, row, index) { // 密码重置
                    event.stopPropagation();
                    event.preventDefault();
                }, 'click .edit': function (event, value, row, index) { // 编辑
                    event.stopPropagation();
    ...
    // modalFooterCancel框关闭按钮事件
    modalFooterCancel.unbind('click').bind('click', function (event) {
        event.stopPropagation();
        event.preventDefault();
        window.$('#batch-import--tool').modal('hide');
    });

    // modalFooterConfirm框关闭按钮事件
    modalFooterConfirm.unbind('click').bind('click', function (event) {
        event.stopPropagation();
        event.preventDefault();
        window.$('#batch-import--tool').modal('hide');
    });
    ...
    window.$('#file-info .modal-footer button.confirm').unbind('click').bind('click', function (event) {
        ...
        if (locationId !== currentOperationVideo['equipmentId']) {
            $http.handle('fileManage_fileInfoEdit', {
                accountId: currentOperationVideo['accountId'],
    ...
    window.$('#file-delete').unbind('click').bind('click', function (event) {
        ...
        $http.handle('userManage_userLogoutInfo', {accountId: reqData}, undefined, undefined, function (resp) {
    ...
    window.$('#file-manage-delete-confirm').unbind('click').bind('click', function (event) {
        ...
        $http.handle('userManage_userLogout', {accountId: reqData}, undefined, undefined, function (resp) {
            $table.refresh(); // 删除完了之后需要刷新表格数据
            $alert.success('<p>成功注销用户<b>' + reqData.length + '</b>个</p>');
